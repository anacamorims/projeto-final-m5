...
export default function SignIn() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://projeto-final-m5-api.onrender.com/api/login', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className={style.container_form}>
      ...
      {error && <p className={style.error_message}>{error}</p>} {/* Exibe mensagem de erro */}
      ...
      <form onSubmit={handleSubmit}>
        ...
        <ButtonDefault name={loading ? "Signing In..." : "Sign In"} disabled={loading} onClick={handleSubmit} />
      </form>
    </div>
  );
}